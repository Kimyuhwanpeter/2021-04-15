...
class LinearDecay_(tf.keras.optimizers.schedules.LearningRateSchedule):
    # if `step` < `step_decay`: use fixed learning rate
    # else: linearly decay the learning rate to zero

    def __init__(self, initial_learning_rate, total_steps, step_decay):
        super(LinearDecay_, self).__init__()
...
lr_scheduler = LinearDecay_(FLAGS.lr, FLAGS.epochs * len_dataset, 15 * len_dataset)  # 이 부분은 꼭 다시 고치자!!
...
def cal_loss(model, images, age_labels, gener_labels, all_age_label, all_age_onehot):
    ...
    with tf.GradientTape() as tape:
        ...
        age_feature = tf.nn.sigmoid(age_feature)
        ...
        logits2 = tf.nn.sigmoid(logits2)
        all_age_loss = -tf.reduce_sum(all_age_onehot * tf.math.log(logits2 + 0.000001) + (1 - all_age_onehot) * tf.math.log(1 - logits2 + 0.000001), 1)
...
def main():
    model = original_GL_network(input_shape=(FLAGS.img_height, FLAGS.img_width, 1), weight_decay=0.00001)
    ...
    if FLAGS.train:
        ...
        train_log_dir = FLAGS.graphs + current_time + "_Adam" + '/train'
        train_summary_writer = tf.summary.create_file_writer(train_log_dir)

        val_log_dir = FLAGS.graphs + current_time + "_Adam" + '/val'
